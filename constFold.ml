...
let memi x env =
  try (match fst(M.find x env) with Int(_) -> true | _ -> false)
  with Not_found -> false
let memf x env =
  try (match fst(M.find x env) with Float(_) -> true | _ -> false)
  with Not_found -> false
let memt x env =
  try (match fst(M.find x env) with Tuple(_) -> true | _ -> false)
  with Not_found -> false

let findi x env = (match fst(M.find x env) with Int(i) -> i | _ -> raise Not_found)
let findf x env = (match fst(M.find x env) with Float(d) -> d | _ -> raise Not_found)
