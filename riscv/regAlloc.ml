...
let p = snd exp in
match fst exp with  
| Nop | Flabel _| Set _ | SetL _ | Comment _ | Restore _ -> (Ans(exp), regenv)
  ...
  | Mul(x, y) -> (Ans((Mul(find x Type.Int regenv, find y Type.Int regenv)),p), regenv)
  | Div(x, y) -> (Ans((Div(find x Type.Int regenv, find y Type.Int regenv)),p), regenv)
