...
class Core extends Module {
  ...
  //即値を取り出す
  val imm_i = inst(31,20) //i形式命令
  val imm_s = Cat(inst(31,25),inst(11,7)) //s形式の即値
  //即値の計算
  val imm_i_sext = Cat(Fill(20,imm_i(11)),imm_i) //offsetを上位ビットで埋めて符号拡張する,IDステージ
  val imm_s_sext = Cat(Fill(20,imm_s(11)),imm_s) //埋め方はI形式と同じ
  ...
  printf(p"dmem.wen : 0x${Hexadecimal(io.dmem.wen)}\n")
  printf(p"dmem.wdata   : 0x${Hexadecimal(io.dmem.wdata)}\n")
  printf(p"gp : ${regfile(3)}\n")
  printf("-------------------\n")
  io.exit := (inst === 0x44.U(WORD_LEN.W))
}