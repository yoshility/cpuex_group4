...
/*class Core extends Module{
    ...
    val imm_j_sext = Cat(Fill(11,imm_j(11)),imm_j,0.U(1.U))
    //U形式
    ...
    alu_out := MuxCase(0.U(WORD_LEN.W),Seq(
        ...
        (exe_fun === ALU_SLL) -> (op1_data << op2_data(4,0))(31,0),//論理左シフト、自然にビット拡張されるため右32ビットぶんを切り出す
        (exe_fun === ALU_SRL) -> (op1_data >> op2_data(4,0)).asUInt(),//論理右シフト、明示的にUInt型へ変換が必要
    ...
    br_flg := MuxCase(false.B,Seq(
        ...
        (exe_fun === BR_BGE) -> !(op1_data.asUInt() < op2_data.asUInt()),
...
class Core extends Module {
  ...
  // Debug
  io.gp   := regfile(3)
  io.exit := (pc_reg === 0x6c.U(WORD_LEN.W))
