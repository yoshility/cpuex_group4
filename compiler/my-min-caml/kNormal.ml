...
| Syntax.FNeg(e,p) ->
  ...
  | Syntax.Let((x, t), e1, e2,p) ->
      ...
      (Let((x, t), e1', e2'),p), t2
  | Syntax.Var(x,p) when M.mem x !Record_global.global2address ->  
    (* Printf.fprintf stdout "global in knormal : %s -> %d\n" x (M.find x !Record_global.global2address); *)
    (Int(M.find x !Record_global.global2address),p), M.find x !Record_global.global2type
  ...
  | Syntax.LetRec({ Syntax.name = (x, t); Syntax.args = yts; Syntax.body = e1 }, e2,p) ->
    let back_up_global = !Record_global.global2address in
    Record_global.global2address := List.fold_left (fun env (x,_) -> M.remove x env) !Record_global.global2address yts;
      let env' = M.add x t env in
      let e1', t1 = g (M.add_list yts env') e1 in
      Record_global.global2address:=back_up_global;
...
in 
  ...
  let rec effect  xx =  (* ???ě?p?Ě?L?ł (caml2html: elim_effect) *)
match fst xx with 
  | Let(_, e1, e2) | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2) -> effect  e1 || effect  e2
  | LetTuple(_, _, e) -> effect  e
  | LetRec({ name = (x, t); args = yts; body = e1 }, e) ->
    effect e
  | ExtFunApp(x,_) 
  when (x = "create_array" || x = "create_float_array" || x = "print_char" || x = "print_int" || x = "print_newline" || x = "read_int" || x = "read_float")-> true
  | App(x, args) ->true
  | Put _ -> true
  | _ -> false