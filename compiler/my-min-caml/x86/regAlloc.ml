...
try g' dest cont regenv exp
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
     g dest cont regenv (Let((x, t), (Restore(x),p), Ans(exp))))
and g' dest cont regenv xx=  (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
let p = snd xx in
match fst xx with  
| Nop | Set _ | SetL _ | Comment _ | Restore _ as exp -> (Ans((exp,p)), regenv)
  ...
  | IfEq(x, y', e1, e2) as exp -> g'_if dest cont regenv (exp,p) (fun e1' e2' -> (IfEq(find x Type.Int regenv, find' y' regenv, e1', e2'),p)) e1 e2
  | IfLE(x, y', e1, e2) as exp -> g'_if dest cont regenv (exp,p) (fun e1' e2' -> (IfLE(find x Type.Int regenv, find' y' regenv, e1', e2'),p)) e1 e2
  | IfGE(x, y', e1, e2) as exp -> g'_if dest cont regenv (exp,p)(fun e1' e2' -> (IfGE(find x Type.Int regenv, find' y' regenv, e1', e2'),p)) e1 e2
  | IfFEq(x, y, e1, e2) as exp -> g'_if dest cont regenv (exp,p) (fun e1' e2' -> (IfFEq(find x Type.Float regenv, find y Type.Float regenv, e1', e2'),p)) e1 e2
  | IfFLE(x, y, e1, e2) as exp -> g'_if dest cont regenv (exp,p) (fun e1' e2' -> (IfFLE(find x Type.Float regenv, find y Type.Float regenv, e1', e2'),p)) e1 e2
  | CallCls(x, ys, zs) as exp ->
      if List.length ys > Array.length regs - 1 || List.length zs > Array.length fregs then
        ...
        g'_call dest cont regenv (exp,p) (fun ys zs -> (CallCls(find x Type.Int regenv, ys, zs),p)) ys zs
  | CallDir(Id.L(x), ys, zs) as exp ->
      if List.length ys > Array.length regs || List.length zs > Array.length fregs then
        failwith (Format.sprintf "cannot allocate registers for arugments to %s" x)
      else
        g'_call dest cont regenv (exp,p) (fun ys zs -> (CallDir(Id.L(x), ys, zs),p)) ys zs
